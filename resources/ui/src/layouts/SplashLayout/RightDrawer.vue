        ...
        methods: {
            chooseImage () {
                this.$root.$emit('show-upload-dialog', {
                    type: 'image',
                    _cb: data => {
                        let maxScale = Math.max(data.width / this.splash.width, data.height / this.splash.height)
                        if (maxScale > 1) {
                            data.scale = 1 / maxScale;
                        }
                        this.$store.commit('Splash/addObject', new Image(data))
                    }
                })
            }
        },
